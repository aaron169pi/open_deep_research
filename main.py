...
from prompts import planner_prompt, code_generation_prompt, code_validation_prompt, file_structure_prompt, file_changes_prompt
   ...
   while True:
      ...
      for batch in batch_files(code_changes_data, batch_size=4):
         ...
         validated_code_response = validation_agent.invoke(
            {"messages": [{"role": "user", "content": f"Entire file changes: {str(code_changes_data)}\n\n\nFiles you need to generate: {batch}\n\nPreviously generated code: {code_context}"}]}
         )
         updated_code_data = []
         ...
         if updated_code_data:
            ...
            for file in updated_code_data:
               if file["content"] == "TERMINATE" or file["content"] == "":
                  ...
                  existing_files[file["file_path"]] = {"file_path": file["file_path"], "content": file["content"], "summary": file["content"] or "" }
            ...
            print_info("There was no update in this batch, please check generated input")
      
      
      commit_changes(base_dir, message=f"Applied user request: {user_input}")


idea = "I want to build a react application that lets people draw shapes and play with them and change their color, etc for fun and creating nice paintings"
